...
local FORMAT_SPECIFIER_FLOAT = "^:%.(%d+)$"
	...
	while i <= len do
		...
		if openBrace == nil then
			...
			if charAfterBrace == "{" then
				...
				elseif formatSpecifier == ":x" then
					assert(typeof(arg) == "number", "received hexadecimal format for " .. typeof(arg))
					buffer:writeRaw(string.format("%x", arg))
				elseif formatSpecifier:match(FORMAT_SPECIFIER_FLOAT) then
					local places = formatSpecifier:match(FORMAT_SPECIFIER_FLOAT)

					assert(typeof(arg) == "number", "received decimal place format for " .. typeof(arg))
					buffer:writeRaw(string.format("%." .. places .. "f", arg))
				else
